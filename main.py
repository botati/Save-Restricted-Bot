...
def handle_private(message, chatid, msgid):
    ...
    try:
        # For public channels (username) use the bot, for private (ID) use the helper
    ...
    thumb = None
    try:
        if msg.video: thumb = downloader.download_media(msg.video.thumbs[0].file_id)
        elif msg.document: thumb = downloader.download_media(msg.document.thumbs[0].file_id)
